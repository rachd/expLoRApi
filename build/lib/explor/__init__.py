...
def recommend_decks():
    player_stats = requests.get(
        'http://ec2-54-85-199-0.compute-1.amazonaws.com/api/players/stats?player_name='+player_id).json()
    player_cards = list(player_stats["stats"]["cards"].keys())
    top_decks = ['CEBAIAIFAEHSQNQIAEAQGDAUDAQSOKJUAIAQCBI5AEAQCFYA']#requests.get('').json()   FETCH TOP 100 DECKS
    top_all_cards = [] #requests.get('').json() FETCH TOP 100 DECKS WHERE ALL CARDS IN PLAYER's CARDS
    top_recommendations = []#recommend_decks(player_cards, top_decks)
    all_cards_recommendations = recommend_decks(player_cards, top_all_cards)
    return json.dumps({'top_decks': top_recommendations, 'player_cards': all_cards_recommendations})